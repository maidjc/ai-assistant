...
def create_user(username, password):
    ...
    cur.execute("INSERT INTO users(username, password_hash, create_time) VALUES(?,?,?)",
                (username, make_hash(password), now))
    conn.commit()
    conn.close()
...
def add_sql(table, data, user_id):
    ...
    cur = conn.cursor()
    if table == "book":
        cur.execute("INSERT INTO book_record(user_id,book_name,author,content,create_time) VALUES(?,?,?,?,?)",
                    (user_id, *data, now))
    elif table == "name":
        cur.execute("INSERT INTO name_record(user_id,type,desc,result,create_time) VALUES(?,?,?,?,?)",
                    (user_id, *data, now))
    elif table == "art":
        cur.execute("INSERT INTO art_record(user_id,style,scene,content,create_time) VALUES(?,?,?,?,?)",
                    (user_id, *data, now))
    elif table == "chat":
        cur.execute("INSERT INTO chat_record(user_id,ask,reply,create_time) VALUES(?,?,?,?)",
                    (user_id, *data, now))
    conn.commit()
    conn.close()
...
def del_sql(table, rid, user_id):
    ...
    cur = conn.cursor()
    if table == "book":
        cur.execute("delete from book_record where id=? and user_id=?", (rid, user_id))
    elif table == "name":
        cur.execute("delete from name_record where id=? and user_id=?", (rid, user_id))
    elif table == "art":
        cur.execute("delete from art_record where id=? and user_id=?", (rid, user_id))
    elif table == "chat":
        cur.execute("delete from chat_record where id=? and user_id=?", (rid, user_id))
    conn.commit()
    conn.close()
...
if st.session_state.user_id is None:
    ...
    with tab1:
        ...
        if st.button("登录", type="primary"):
            ...
            if row and check_hash(password, row[2]):
...
if func == "💬 对话":
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            st.markdown(ans)
        add_sql("chat", [prompt, ans], user_id)
...
# -------------------------- 2.书摘模块 --------------------------
    ...
    if st.button("📚 获取介绍", type="primary") and book_name:
        with st.spinner("整理内容中..."):
            ...
            st.markdown(ans)
            add_sql("book", [book_name, author, ans], user_id)
...
# --------------------------3.起名模块 --------------------------
    ...
    if st.button("✨生成名字",type="primary") and desc:
        with st.spinner("构思中..."):
            ...
            ans = client.chat.completions.create(model=MODEL_NAME,messages=[{"role":"system","content":SYSTEM_PROMPT},{"role":"user","content":ask}]).choices[0].message.content
            st.markdown(ans)
            add_sql("name",[typ,desc,ans], user_id)
...
# --------------------------4.朋友圈文案 --------------------------
    ...
    if st.button("✨生成文案",type="primary") and scene:
        with st.spinner("撰写文案..."):
            ...
            ans = client.chat.completions.create(model=MODEL_NAME,messages=[{"role":"system","content":SYSTEM_PROMPT},{"role":"user","content":ask}]).choices[0].message.content
            st.markdown(ans)
            add_sql("art",[style,scene,ans], user_id)
...
# --------------------------5.我的存档 --------------------------
    ...
    with tab1:
        ...
        data = search_sql("book",kw,user_id)
        for row in data:
            st.write(f"📅{row[5]}｜{row[2]}【{row[3]}】")
            st.text(row[4])
            if st.button("删除",key=f'b{row[0]}'):
                del_sql("book",row[0],user_id)
                st.rerun()
    #起名
    with tab2:
        kw = st.text_input("需求关键词")
        data = search_sql("name",kw,user_id)
        for row in data:
            st.write(f"📅{row[5]}｜分类：{row[2]}｜需求：{row[3]}")
            st.text(row[4])
            if st.button("删除",key=f'n{row[0]}'):
                del_sql("name",row[0],user_id)
                st.rerun()
    #文案
    with tab3:
        kw = st.text_input("场景搜索")
        data = search_sql("art",kw,user_id)
        for row in data:
            st.write(f"📅{row[5]}｜风格：{row[2]}｜场景：{row[3]}")
            st.text(row[4])
            if st.button("删除",key=f'a{row[0]}'):
                del_sql("art",row[0],user_id)
                st.rerun()
    #聊天
    with tab4:
        kw = st.text_input("提问检索")
        data = search_sql("chat",kw,user_id)
        for row in data:
            st.write(f"📅{row[4]} 用户：{row[2]}")
            st.text(f"小政：{row[3]}")
            if st.button("删除",key=f'c{row[0]}'):
                del_sql("chat",row[0],user_id)
                st.rerun()
